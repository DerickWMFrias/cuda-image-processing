...
__global__ void modify_hue_kernel(png_bytep d_image,
    ...
    int i_ = (blockIdx.y * blockDim.y) + threadIdx.y;
    int j_ = (blockIdx.x * blockDim.x) + threadIdx.x;

    if((i_ < height) && (j_ < width)){
        png_bytep row = &(d_image[i_ * width * 3]);
        png_bytep px = &(row[j_ * 3]);
        modify_pixel(px, A);
    }
}
...
void modify_hue(png_bytep h_image,
    ...
    double* cuda_A;
    png_bytep cuda_h_image;

    //fprintf(stderr, "%d %d\n",           sizeof(png_bytep), sizeof(h_image));
    ...
    cudaMalloc(&cuda_A, 9*sizeof(double));
    checkErrors(cudaGetLastError(), "Alocacao da matriz A no device");

    cudaMemcpy(cuda_A, A, 9*sizeof(double), cudaMemcpyHostToDevice);
    checkErrors(cudaGetLastError(), "Copia da matriz A para o device");

    cudaMalloc(&cuda_h_image, image_size);
    checkErrors(cudaGetLastError(), "Alocacao da imagem no device");

    cudaMemcpy(cuda_h_image, h_image, image_size, cudaMemcpyHostToDevice);
    ...
    int block_measure = 16;
    int grid_measure = (int) ceil(mx / block_measure);

    dim3 dim_block(block_measure, block_measure);
    dim3 dim_grid(grid_measure, grid_measure);

    modify_hue_kernel<<<dim_grid, dim_block>>>(cuda_h_image, width, height, cuda_A);
    checkErrors(cudaGetLastError(), "Lançamento do kernel");

    cudaMemcpy(h_image, cuda_h_image, image_size, cudaMemcpyDeviceToHost);
    checkErrors(cudaGetLastError(), "Copia da imagem para o host");

    cudaFree(cuda_h_image);
    cudaFree(cuda_A);
}
